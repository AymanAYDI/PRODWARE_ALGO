codeunit 50007 "VDOC - Manuel Export Xls Multi"
{
    ...
    // //----------------------------------------------------------
    // //----------------------------------------------------------
    // // 16/09/2022 : Modification pour Moovapps - 
    // // Ajout d'un filtre pour exclure les lignes de receptions n'ayant pas de n° d'imputation renseignée
    // // Ligne 207 : RecGPurchaseLine.SETFILTER(RecGPurchaseLine."WMS No Imputation", '');   
    // //----------------------------------------------------------
    // //----------------------------------------------------------
    ...
    begin
        ...
        // 20221012 -> Remettre en PROD : RecGPurchaseLine.SETRANGE("Exported Line", FALSE);

        // check si la ligne CA correspond à une commande DEV : si DEV alors exclue
        RecGPurchaseLine.SETRANGE("Development Order", FALSE);

        // 20221012 -> ajout du Filtre pour exclure les n° d'imputation WMS ( <> '' ne fonctionne pas )
        RecGPurchaseLine.SETFILTER(RecGPurchaseLine."WMS No Imputation", '<1&<>/');

        // Check sur la date de comptabilisation : uniquement les Lig Rcpt Ach Enreg depuis le 01/01/2019
        // yyyy-mm-ddD 090418D
        // 20221012 -> RecGPurchaseLine.SETFILTER(RecGPurchaseLine."Posting Date", '>%1', 20190101D);
        RecGPurchaseLine.SETFILTER(RecGPurchaseLine."Posting Date", '>%1', 20220901D);
    ...
    begin
        ...
        //20221012 - A remettre en PROD --> TempReceipLine.SETRANGE("Exported Line", FALSE);
