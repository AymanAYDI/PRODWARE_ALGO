report 50015 "Check - ALGO"
{
    // version NAVW113.00,NAVFR13.00
    // copy from R1401

    DefaultLayout = RDLC;
    RDLCLayout = 'Reports/Rep50015.CheckALGO.rdl';
    Caption = 'Check';
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            column(PostingDate_GenJnLine; "Posting Date")
            {
            }
            column(DocNo_GenJnLine; "Document No.")
            {
            }
            ...
            dataitem(CheckPages; "Integer")
            {
                ...
                column(Number_CheckPages; Number)
                {
                }
                ...
                column(DescriptionLine_1___; DescriptionLine[1] + ' *****')
                {
                }
                column(CheckToAddr_1___; CheckToAddr[1] + '*******')
                {
                }
                column(CheckAmountText___; '***' + CheckAmountText + '***')
                {
                }
                column(DescriptionLine_1____; '                     ' + (DescriptionLine[1]) + ' *****')
                {
                }
                column(Le_Service_Comptabilité_Caption; Le_Service_Comptabilité_CaptionLbl)
                {
                }
                column(Nous_vous_prions__Madame__Monsieur_d_agréer_nos_salutations_distinguées_Caption; Nous_vous_prions__Madame__Monsieur_d_agréer_nos_salutations_distinguées_CaptionLbl)
                {
                }
                column(SHEDULE_OF_PAYMENTCaption; SHEDULE_OF_PAYMENTCaptionLbl)
                {
                }
                column(en_règlement_de_vos_factures_référencées_ci_après_Caption; en_règlement_de_vos_factures_référencées_ci_après_CaptionLbl)
                {
                }
                column(FORMAT_TODAY_0_4_; DELCHR(FORMAT(TODAY, 0, 4), '=', '.'))
                {
                }
                column(theCaption; theCaptionLbl)
                {
                }
                column(CompanyInfo_City; CompanyInfo.City)
                {
                }
                column(Vend__Country_Region_Code_; Vend."Country/Region Code")
                {
                }
                ...
                column(Vend__Address; Vend."Address")
                {
                }
                column(Vend_Name; Vend.Name)
                {
                }
                column(STRSUBSTNO_Text067_CompanyInfo__Post_Code__CompanyInfo_City_; STRSUBSTNO(Text067, CompanyInfo."Post Code", CompanyInfo.City))
                {
                }
                column(CompanyInfo__Trade_Register_; CompanyInfo."Trade Register")
                {
                }
                column(Registre_du_commerceCaption; Registre_du_commerceCaptionLbl)
                {
                }
                column(STRSUBSTNO_Text066_CompanyInfo_Address___CompanyInfo__Address_2__; STRSUBSTNO(Text066, CompanyInfo.Address, CompanyInfo."Address 2"))
                {
                }
                column(CompanyInfo__Registration_No__; CompanyInfo."Registration No.")
                {
                }
                column(SIRET_n_Caption; SIRET_n_CaptionLbl)
                {
                }
                column(APE_Code_Caption; APE_Code_CaptionLbl)
                {
                }
                column(SARL_au_capital_de_500_000_EurosCaption; SARL_au_capital_de_500_000_EurosCaptionLbl)
                {
                }

                column(STRSUBSTNO_Text065_CompanyInfo__Phone_No___CompanyInfo__Fax_No___; STRSUBSTNO(Text065, CompanyInfo."Phone No.", CompanyInfo."Fax No."))
                {
                }
                ...
                column(Veuillez_trouver_ci_dessous_le_chèque_n_Caption; Veuillez_trouver_ci_dessous_le_chèque_n_CaptionLbl)
                {
                }
                column(Vend__No__; Vend."No.")
                {
                }
                column(CompanyInfoAPECodeRTC; CompanyInfo."APE Code")
                {
                }
                dataitem(PrintSettledLoop; "Integer")
                {
                    ...
                    column(Notre_RéférenceCaption; Notre_RéférenceCaptionLbl)
                    {
                    }
                    column(Date_DocumentCaption; Date_DocumentCaptionLbl)
                    {
                    }
                    column(GenJnlLinLineNo; GenJnlLine."Line No.")
                    {
                    }
                    column(CheckNoText_; CheckNoText)
                    {
                    }
                    ...
                    column(Total_EUR_Chèque_n_Caption; Total_EUR_Chèque_n_CaptionLbl)
                    {
                    }
                ...
                dataitem(PrintCheck; "Integer")
                {
                    DataItemTableView = SORTING(Number);
                    MaxIteration = 1;
                    // column(Number_PrintCheck; Number)
                    // {
                    // }
                    ...
                    column(CarcassonneCaption; CarcassonneCaptionLbl)
                    {
                    }
                    column(CheckNoTextRTC; CheckNoText)
                    {
                    }
                    // column(CheckToAddr_1_____________; CheckToAddr[1] + '*******')
                    // {
                    // }
                    // column(CheckDateText_; CheckDateText)
                    // {
                    // }
    ...
    var
        ...
        Le_Service_Comptabilité_CaptionLbl: Label 'Le Service Comptabilité.';
        Nous_vous_prions__Madame__Monsieur_d_agréer_nos_salutations_distinguées_CaptionLbl: Label 'Nous vous prions, Madame, Monsieur d agréer nos salutations distinguées.';
        SHEDULE_OF_PAYMENTCaptionLbl: Label 'BORDEREAU DE REGLEMENT';
        en_règlement_de_vos_factures_référencées_ci_après_CaptionLbl: Label 'en règlement de vos factures référencées ci-après.';
        theCaptionLbl: Label ', le';
        Registre_du_commerceCaptionLbl: Label 'Registre du commerce';
        SIRET_n_CaptionLbl: Label 'No. SIRET';
        APE_Code_CaptionLbl: Label 'Code APE :';
        SARL_au_capital_de_500_000_EurosCaptionLbl: Label 'SARL au capital de 500 000 Euros';
        Veuillez_trouver_ci_dessous_le_chèque_n_CaptionLbl: Label 'Veuillez trouver ci-dessous le chèque no.';
        CarcassonneCaptionLbl: Label 'Carcassonne';
        Notre_RéférenceCaptionLbl: Label 'Notre Référence';
        Date_DocumentCaptionLbl: Label 'Date document';
        Total_EUR_Chèque_n_CaptionLbl: Label 'Total EUR Chèque no.';

        "--MIGRATION2009--": TextConst;
        Text066: Label 'FRA="%1 - %2 "';
        Text067: Label 'FRA=%1   %2 - France';
        Text065: Label 'FRA=T‚l : %1  -  Fax : %2';
        "-MIGRATION2009R2-": Integer;
        DecGLineAmount2: Decimal;
    ...
    begin
        ...
        //>>MIGRATION2009R2
        TxtGCompanyPhoneFaxEmail := 'Phone: ' + CompanyInfo."Phone No.";
        TxtGCompanyAdress := CompanyInfo.Address;
        //<<MIGRATION2009R2
    end;
