report 50033 "Sales - Invoice - A2"
{
    // version NAVW113.01,NAVFR13.01
    //Copy from R206

    DefaultLayout = RDLC;
    RDLCLayout = 'Reports/Rep50033.SalesInvoiceA2.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            //>>ALGO
            column(TxtPaymentMethod; TxtPaymentMethod)
            {
            }
            column(RecGPaymentModeDescription; RecGPaymentMode.Description)
            {
            }
            column(TxtBankIdentifier; TxtBankIdentifier)
            {
            }
            column(BankAccountNo; RecGBankAccount."Bank Account No.")
            {
            }
            column(TxtGHeader01; STRSUBSTNO(TxtGHeader, CompanyInfo."EORI No.", CompanyInfo."MID Code", CompanyInfo."AEO Certificate No."))
            {
            }
            column(TextFrTaxe; TextFrTaxe)
            {
            }
            column(TextCEE; TextCEE)
            {
            }
            column(TextHORSCEE; TextHORSCEE)
            {
            }
            column(CodTermPrice; CodTermPrice)
            {
            }
            column(BooGRetrocession; BooGRetrocession)
            {
            }
            ...
            // ALGO du 20190913
            column(GrpcptamarchTVa; cust."VAT Bus. Posting Group")
            {
            }
            //<<ALGO
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    // ALGO20200317 - ajout de la forme legal pour réutilisation en pieds de page
                    column(TxtFooter01; STRSUBSTNO(TxtGFooter01, CompanyInfo."Stock Capital", CompanyInfo."Registration No.", CompanyInfo."APE Code", CompanyInfo."VAT Registration No.", CompanyInfo."Trade Register", CompanyInfo."Legal Form"))
                    {
                    }
                    // ALGO20200317 - ajout de la forme legal pour réutilisation en pieds de page                    
                    ...
                    column(TxtGFooter03; TxtGFooter03)
                    {
                    }
                    // ALGO20200317 - ajout d'une nouvelle ligne de pied de page pour le siege social
                    column(TxtGFooter04; STRSUBSTNO(TxtGFooter04, ' ' + CompanyInfo.Name, ' ' + CompanyInfo."Head Office"))
                    {
                    }
                    // ALGO20200317 - ajout d'une nouvelle ligne de pied de page pour le siege social                    
                    ...
                    column(TxtGFooter1; TxtGFooter1)
                    {
                    }
                    column(TxtGFooter2; TxtGFooter2)
                    {
                    }
                    column(TxtGCustomer; TxtGCustomer)
                    {
                    }
                    column(TxtSignature; TxtSignature)
                    {
                    }
                    column(TxtDate; TxtDate)
                    {
                    }
                    column(TxtPlugCompany; TxtPlugCompany)
                    {
                    }
                    column(TxtCstG001; TxtCstG001)
                    {
                    }
                    column(TxtCstG002; TxtCstG002)
                    {
                    }
                    column(TxtCstG003; TxtCstG003)
                    {
                    }
                    column(TxtCstG004; TxtCstG004)
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(SalesLinePO; "Sales Invoice Line".PO)
                        {
                        }
                        column(Order_No_; "Order No.")
                        {
                        }
                        column(NetWeight; StrSubstNo(TxtNetWeight, RecGItem."Weight (Net/Gross)"))
                        {
                        }
                        ...
                        column(TotalNetWeightValue; RecGItem."Weight (Net/Gross)" * Quantity)
                        {
                        }
                        column(CountryOfOrigine; STRSUBSTNO(TxtGOrigin, RecGCountry.Name))
                        {
                        }
                        column(ItemTariffNo; StrSubstNo(TxtHSCode, RecGItem."Tariff No."))
                        {
                        }
                        column(ItemTariffNoCaption; RecGItem.FieldCaption("Tariff No."))
                        {
                        }
                        column(ItemHeight; RecGItem."Height (cm)")
                        {
                        }
                        column(ItemLenght; RecGItem."Length (cm)")
                        {
                        }
                        column(ItemWidth; RecGItem."Width (cm)")
                        {
                        }
                        column(Dimension; StrSubstNo(TxtDimension, RecGItem."Height (cm)", RecGItem."Length (cm)", RecGItem."Width (cm)"))
                        {
                        }
                        column(Composition; StrSubstNo(TxtOutsideComposition, RecGItem."Percentage Composition 1", Format(RecGItem.Composition)))
                        {
                        }
                        column(Leather; StrSubstNo(TxtLeatherTrim, RecGItem."Percentage Composition 2", format(RecGItem.Leather)))
                        {
                        }
                        column(Linning; StrSubstNo(TxtInsideLinning, format(RecGItem.Linning)))
                        {
                        }
                        column(MetalPart; StrSubstNo(TxtMetalPart, format(RecGItem."Metal parts")))
                        {
                        }
                        column(DecGQuantity; DecGQuantity)
                        { }
                        column(DecGWeight; DecGWeight)
                        { }
                        column(Wood1; RecGItem."Wood 1")
                        { }
                        column(Wood2; RecGItem."Wood 2")
                        { }
                        column(Wood3; RecGItem."Wood 3")
                        { }
                        ...
                        //>>ALGO
                        var
                            RecLItemTrans: Record "Item Translation";
                        //<<ALGO
                        begin
                            //>>ALGO
                            if (Quantity = 0) and Not (Type = Type::" ") then
                                CurrReport.Skip();
                            //<<ALGO
                            ...
                            IF (RecGItem.FIND('-')) THEN BEGIN
                                RecLItemTrans.RESET();
                                RecLItemTrans.SETRANGE("Item No.", RecGItem."No.");
                                RecLItemTrans.SETRANGE("Language Code", "Sales Invoice Header"."Language Code");
                                IF RecLItemTrans.FIND('-') THEN
                                    TxtGColor := RecLItemTrans."Description 2"
                                ELSE
                            ...
                            // Origine
                            RecGCountry.RESET();
                            if RecGCountry.GET(RecGItem."Country/Region of Origin Code") then;
                            // End Origine     
                            DecGQuantity += Quantity;
                            DecGWeight += Quantity * RecGItem."Weight (Net/Gross)";
            ...
            begin
                ...
                IF NOT RecGBankAccount.FIND('-') THEN BEGIN
                    RecGBankAccount.SETFILTER(RecGBankAccount."Currency Code", '%1', '');
                    IF RecGBankAccount.FIND('-') THEN;
                END;

                IF ShipmentMethod.GET("Sales Invoice Header"."Shipment Method Code") THEN
                    CodTermPrice := TxtShipmentMethod + ShipmentMethod.Code + ' - ' + ShipmentMethod.Description
                else
                    CodTermPrice := '';
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    //>>ALGO
                    field(BooGRetrocession; BooGRetrocession)
                    {
                        ApplicationArea = ALL;
                        Caption = 'Retrocession';
                    }
                    //<<ALGO
    ...
    var
        ...
        RecGPaymentMode: Record "Payment Method";
        ...
        //>>ALGO
        TxtGColor: Text[50];
        CodTermPrice: Text[100];
        DecGQuantity: Decimal;
        DecGWeight: Decimal;

        TxtGFax: Label '-Fax :';
        TxtGTotRef: Label 'Reference Total   :';
        TxtGFooter1: Label 'The 2 parts signature will validate the order :';
        TxtGFooter2: Label 'under the enumerated conditions of sale';
        ...
        TxtGFooter03: Label 'Fixed compensation of 40€ corresponding to collecting charges in the case the payment is made after the due date (Article L.441-3 and L.441-6 of the French commercial code)';
        // ALGO201200317 - ajout ligne pied de page siege social
        TxtGFooter04: Label 'Head Office : %1 %2';
        // ALGO201200317 - ajout ligne pied de page siege social        
        TxtGCustomer: Label 'Client: ';
        TxtDate: Label 'Date :';
        TxtSignature: Label 'Signature :';
        TxtPlugCompany: Label 'Plug Company :';
        TxtCstG001: Label 'Color :';
        TxtCstG002: Label 'Composition :';
        TxtCstG003: Label 'We certify ................. see general conditions of sales';
        TxtCstG004: Label 'Order Reference';
        TxtPaymentMethod: Label 'Payment Method :';
        TxtGOrigin: Label 'Country Of Origine : %1';
        ...
        TxtDimension: Label 'Dimension (H*L*W) : %1, %2, %3';
        ...
        TxtBankIdentifier: Label 'Bank Identifier No. :';
        TxtGHeader: Label 'N° EORI : %1 - Code MID : %2 - Certificat AEO N° : %3';
        TextFrTaxe: Label 'Vente en suspension de taxe en application de l''article 275-I du CGI';
        TextCEE: Label 'Exonération de TVA en application de l''article %1 du CGI';
        TextHORSCEE: Label 'Exonération de TVA en application de l''article 262-I 1er du CGI';
        TxtShipmentMethod: Label 'Shipment Method : ';
