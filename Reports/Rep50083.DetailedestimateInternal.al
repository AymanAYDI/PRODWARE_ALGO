report 50083 "Detailed estimate - Internal"

// Ajout du 20191017 - DSP - AL - Ajout des champs Position et Position 2 dans le report au niveau de la BOM
// Ajout du 20191017 - DSP - RDL - Modification de tous les champs à decimal sur 2 sauf pour les quantités dans la BOM
// Ajout du 20191017 - DSP - RDL - Centrage de L'exwork actif entre CUB et CUR car donnée commune dans le pavé CONTROLE
// Ajout du 20191017 - DSP - AL - Ajout du champs grp produit de l'article dans le report 
// Ajout du 20191017 - DSP - RDL - Correction/homogène sur taille espacement des données dans le report
// 20191103 - DSP - Ajout du PARETO et autres modifications


{
    Caption = 'Detailed estimate - Internal';
    DefaultLayout = RDLC;
    RDLCLayout = 'Reports/Rep50083.DetailedestimateInternal.rdl';
    ...
    dataset
    {
...

        {
            ...
            { }
            column(Global_Position2; position2)
            { }
            column(Global_Pareto; Pareto)
            { }
            ...
            dataitem("Routing Line"; "Routing Line")
            {
                ...
                begin
                    ...
                    ProdTotalCostFULL := ProdTotalCostFULL + ProdTotalCost;
                end;
            ...
            dataitem(BOMLoop; Integer)
            {
                ...
                column(BOMLoop_Tauxdemaj; (vCoefMajorBOM - 1) * 100)
                { }
                ...
                { }
                // Ajout du 20191017 - DSP
                column(BOMLoop_Position; Prodbomline[level].Position)
                { }
                column(BOMLoop_Position2; Prodbomline[level]."Position 2")
                { }
                // Ajout du 20191017 - DSP                  
                column(BOMLoop_CUBUnit; ProdCUB."Unit Cost")
                { }
                column(BOMLoop_CUBMaj; ProdBOMLine[Level].Quantity * ProdCUB."Unit Cost" * vCoefMajorBOM)
                { }
                column(BOMLoop_CURUnit; ProdCUR."Unit Cost")
                { }
                column(BOMLoop_CURMaj; DecGCostLineCUR)
                { }
                Column(Item_GenProdPostingGroup; compitem."Gen. Prod. Posting Group")
                { }
                column(item_pareto; Item_pareto)
                { }
                ...
                begin
                    ...
                    // ----------- PARETO ----------
                    IF Prodbomline[level]."Position 2" = '' then Item_pareto := compitem."Gen. Prod. Posting Group";
                    IF Prodbomline[level]."Position 2" <> '' then Item_pareto := Prodbomline[level]."Position 2";
                    ...
                    IF BooGLUNIFORM = TRUE THEN BEGIN
                        ...
                        IF ProdCUR."Unit Cost" <> 0 THEN BEGIN
                            DecGCostLineCUR := (ProdBOMLine[Level].Quantity * ProdCUR."Unit Cost") * vCoefMajorBOM;
                        ...
                        END ELSE BEGIN
                            ...
                            DecGCostLineCUR := (ProdBOMLine[Level].Quantity * ProdCUB."Unit Cost") * vCoefMajorBOM;
                            vInformation :=
            ...
            dataitem(Footer; Integer)
            {
                ...
                column(Footer_DecGCostTotalMaj; DecGCostTotal * vCoefMajorBOM)
                { }
                column(Footer_DecGCostTotalCURMaj; DecGCostTotalCUR * vCoefMajorBOM)
                { }
                // column(Footer_CostFreight; Item."Freight Cost" * vCoefMajor)
                column(CoutTQLLU; (DecGPRALU * (vCoefTql - 1)))
                { }
                column(CoutTQLLUCUR; (DecGPRALUCUR * (vCoefTql - 1)))
                { }
                ...
                { }
                column(Footer_DecGPRALUCUR; DecGPRALUCUR)
                { }
                ...
                begin
                    if BooGLUNIFORM then begin
                        DecGPRALU := (ProdTotalCostFULL * vCoefMajor) + (DecGCostTotal * vCoefMajorBOM);
                        // DecGPRALU := (ProdTotalCost + DecGCostTotal);
                        DecGTotCostLU := DecGPRALU + (DecGPRALU * (vCoefTql - 1));
                        DecGExworksLU := DecGTotCostLU * vCoefExWorks;

                        DecGPRALUCUR := (ProdTotalCostFULL * vCoefMajor) + (DecGCostTotalCUR * vCoefMajorBOM);
                        // DecGPRALUCUR := (ProdTotalCost + DecGCostTotalCUR);
                    ...
                    if BooGGoyard then begin
                        DecGPRAGSH := (ProdTotalCostFULL * vCoefMajor) + (DecGCostTotal * vCoefMajorBOM);
                        // DecGPRAGSH := (ProdTotalCost + DecGCostTotal);
                        DecGTQL := DecGPRAGSH * (vCoefTql - 1);
                        DecGTotCostGSH := DecGPRAGSH + DecGTQL;
                        DecGExworksGSH := ROUND(DecGTotCostGSH * vCoefExWorks, 1);

                        DecGPRAGSHCUR := (ProdTotalCostFULL * vCoefMajor) + (DecGCostTotalCUR * vCoefMajorBOM);
                        // DecGPRAGSHCUR := (ProdTotalCost + DecGCostTotalCUR);
            ...
            begin
                ...
                IF BooGGoyard = TRUE THEN BEGIN
                    ...
                    IF ProdTQL.FINDSET() THEN BEGIN
                        ...
                        vCoefMajorBOM := 1 + (ProdTQL.CoefmajorationNomenclature / 100);
        ...
        layout
        {
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    {
                        ApplicationArea = ALL;
                        Caption = 'Afficher la colonne Sous-Traitance';

                    }
                    field(Position2; Position2)
                    {
                        ApplicationArea = ALL;
                        Caption = 'Afficher la colonne Element';
                    }
                    field(Pareto; Pareto)
                    {
                        ApplicationArea = ALL;
                        Caption = 'Afficher le Pareto';
                    }
    ...
    var
        ...
        CodGItemNo: Code[20];
        CodGBomVersion: Code[20];
        CodGRoutingVersion: Code[20];
