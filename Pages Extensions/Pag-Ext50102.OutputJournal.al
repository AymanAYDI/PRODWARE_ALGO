pageextension 50102 "OutPut Journal" extends "Output Journal"
{
    //p99000823
                ...
                If RecManufacturingSetup."Securisation OF" = true then BEGIN // DEBUT SI SECURISATION ACTIVEE
                    ...
                    IF NumOpeSuivante <> '' THEN BEGIN
                        ...
                        FOR i := 1 TO (NumCommas + 1) DO BEGIN
                            ...
                            RecLItmJnlLineNEXT.setfilter(RecLItmJnlLineNEXT."Journal Template Name", 'PRODUCTION');
                    ...
                    IF NumOpePrecedente <> '' THEN BEGIN
                        ...
                        FOR i := 1 TO (NumCommas + 1) DO BEGIN
                            ...
                            RecLItmJnlLinePREV.setfilter(RecLItmJnlLinePREV."Journal Template Name", 'PRODUCTION');
...
/*
                            ...
                            Message(' DEBUG ON ------------------------------' + FORMAT(chr13) + FORMAT(chr10)
                            + ' - Operation                                  : ' + Format(NumOpeEncours) + FORMAT(chr13) + FORMAT(chr10)
                            + '   Qte Saisie Operation encours               : ' + Format(QteProduiteEncours) + FORMAT(chr13) + FORMAT(chr10)
                            + '   Qte Saisie Operation encours (XREC)        : ' + Format(QteproduiteEncoursXREC) + FORMAT(chr13) + FORMAT(chr10)
                            + ' ---------------------------------------------  ' + FORMAT(chr13) + FORMAT(chr10)
                            + ' - Operation precedente                       : ' + Format(NumOpePrecedente) + FORMAT(chr13) + FORMAT(chr10)
                            + ' - Operation precedente Parcours              : ' + Format(SELECTSTR(i, NumOpePrecedente)) + FORMAT(chr13) + FORMAT(chr10)
                            + ' - Operation precedente Reference             : ' + Format(NumOpePrecedenteReference) + FORMAT(chr13) + FORMAT(chr10)
                            + '   Qte Capa precedente                        : ' + Format(SomQteCapaOpePrecedente) + FORMAT(chr13) + FORMAT(chr10)
                            + '   Qte Saisie Operation precedente            : ' + Format(QteProduiteSaisieOpePrecedente) + FORMAT(chr13) + FORMAT(chr10)
                            + '   Qte Parallele Operation Precedente         : ' + Format(QteParallelePrecedente) + FORMAT(chr13) + FORMAT(chr10)
                            + '   Qte Parallele Saisie Operation precedente  : ' + Format(QteParallelePrecedenteSaisie) + FORMAT(chr13) + FORMAT(chr10)
                            + ' ---------------------------------------------  ' + FORMAT(chr13) + FORMAT(chr10)
                            + ' - Operation suivante                         : ' + Format(NumOpeSuivante) + FORMAT(chr13) + FORMAT(chr10)
                            + ' - Operation Suivante Reference               : ' + Format(NumOpeSuivanteReference) + FORMAT(chr13) + FORMAT(chr10)
                            + '   Qte Capa Suivante                              : ' + Format(SomQteCapaOpeSuivante) + FORMAT(chr13) + FORMAT(chr10)
                            + '   Qte Saisie Operation Suivante              : ' + Format(QteProduiteSaisieOpeSuivante) + FORMAT(chr13) + FORMAT(chr10)
                            + '   Qte Parallele Operation Suivante           : ' + Format(QteParalleleSuivante) + FORMAT(chr13) + FORMAT(chr10)
                            + '   Qte Parallele Saisie Operation Suivante    : ' + Format(QteParalleleSuivanteSaisie) + FORMAT(chr13) + FORMAT(chr10)
                            + ' ---------------------------------------------  ' + FORMAT(chr13) + FORMAT(chr10)
                            + ' - Quantité déjà déclarée opé N              :   ' + Format(OpeCapaEntries) + FORMAT(chr13) + FORMAT(chr10)
                            + ' - Cas opération en cours                    : ' + Format(CasOperation) + FORMAT(chr13) + FORMAT(chr10)
                            + ' ---------------------------------------------  ' + FORMAT(chr13) + FORMAT(chr10)
                            + ' DEBUG END M1 --------------------------------  ');

                    Message(' DEBUG ON M1 -----------------------------' + FORMAT(chr13) + FORMAT(chr10)
                    + ' - OF                                         : ' + Format(NumOFEncours) + FORMAT(chr13) + FORMAT(chr10)
                    + ' - N° Ligne                                     : ' + Format(NumLigneOFEncours) + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Type Ecriture                              : ' + Format("Entry Type") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - N°Ligne Feuil Prod                          : ' + Format(NumLigneOpeEncours) + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Article                                    : ' + Format(CodeArticleEncours) + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Operation                                  : ' + Format(NumOpeEncours) + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Description                                : ' + Format(NomOperationEncours) + FORMAT(chr13) + FORMAT(chr10)
                    + ' ---------------------------------------------  ' + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité Produite utilisateur            :   ' + Format("QteProduiteEncours") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité Produite (XREC)                 :   ' + Format("QteproduiteEncoursXREC") + FORMAT(chr13) + FORMAT(chr10)
                    + ' ---------------------------------------------  ' + FORMAT(chr13) + FORMAT(chr10)
                    + ' DEBUG END M1 --------------------------------  ');


                    Message(' DEBUG ON  -----------------------------------------------' + FORMAT(chr13) + FORMAT(chr10)
                    + ' - OF                                         : ' + "Order No." + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Type Ecriture                              : ' + Format("Entry Type") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - N°Ligne Feul Prod                          : ' + Format("Line No.") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Article                                    : ' + "Item No." + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Operation                                  : ' + Format("Operation No.") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Secteur                                    : ' + Format("No.") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Description                                : ' + Format("Description") + FORMAT(chr13) + FORMAT(chr10)
                    + ' -----------------------------------------------' + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité Totale à Produire sur l''OF       : ' + Format("QteTotaleAProduireOF") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité Restante à Produire sur l''OF     : ' + Format("QteRestanteAProduireOF") + FORMAT(chr13) + FORMAT(chr10)
                    + ' -----------------------------------------------' + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Nom du secteur / Opération encours         : ' + Format("NomOperationEncours") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - N° Opération encours                     : ' + Format(NumOpeEncours) + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité validée pour le secteur encours   : ' + Format("SomQteCapaEncours") + FORMAT(chr13) + FORMAT(chr10)
                    + ' -----------------------------------------------' + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Nom du secteur / Operation Precedente      : ' + Format("NomOperationPrecedente") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - N° Opération Precedente                    : ' + Format("Numopeprecedente") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité validée pour le secteur précédent : ' + Format("SomQteCapaOpePrecedente") + FORMAT(chr13) + FORMAT(chr10)
                    + ' -----------------------------------------------' + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Nom du secteur / Operation Suivante        : ' + Format("NomOperationSuivante") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - N° Opération Suivante                      : ' + Format(NumOpeSuivante) + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité validée pour le secteur Suivant   : ' + Format("SomQteCapaOpeSuivante") + FORMAT(chr13) + FORMAT(chr10)
                    + ' -----------------------------------------------' + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité saisie par l''utilisateur          : ' + Format("QteProduiteEncours") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité  (XREC)                            : ' + Format("QteproduiteEncoursXREC") + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité saisie precedente OPE              : ' + Format(QteProduiteSaisieOpePrecedente) + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Quantité saisie suivante OPE                : ' + Format(QteProduiteOpeSuivante) + FORMAT(chr13) + FORMAT(chr10)
                    + ' -----------------------------------------------' + FORMAT(chr13) + FORMAT(chr10)
                    + ' - Cas opération                              : ' + Format("CasOperation") + FORMAT(chr13) + FORMAT(chr10)
                    + ' DEBUG END -----------------------------------------------');
*/
